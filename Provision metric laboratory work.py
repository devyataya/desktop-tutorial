...
1.   Choose the data you want to analyze: service and territory (write in group chat your choice, it should not repeat someone's else one)

2.   Get the boundaries of your territory (you can use OSMNX Geocoder Module or e.g. find its .geojson file)

3.   Use territory geometry to load residential building on it with OSMNX Features Module (you can use not only polygon method for that)

4.   Look is there ane duplicates. If there are some - drop them

5.   Proccess "levels" attribute of your building to fill None (empty) values and handle different types

6.   Calculate area of the polygon, living area (levels * area * 0.8) and population for each building (living_area / 31)

7. Load the data of your choosen service (keep in mind and check that you can load it with building, amenity tags or any others). Because of it, check geometries for intersections.

8. Apply .buffer method to make buffers around services (if you do not know established distance - use 1km as defalut)

9. Calculate Provision Metric for your service on the territory (for the help on that step - look at the provision calculation from previous colab)
"""
